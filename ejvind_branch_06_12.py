...
app.layout = dbc.Container([
    ...
    dbc.Col(html.Div(dcc.Graph(id="choropleth")), width=8),
    dbc.Col([
        html.Div(dcc.Graph(id="min_max_graph")),
        dcc.Dropdown(
            id="select_year",
    ...
    dbc.Row([
        ...
        # Choropleth Map
        dbc.Col(html.Div(dcc.Graph(id="choropleth")), width=6),
        
        # min max graph
        dbc.Col([html.Div(dcc.Graph(id="min_max_graph")), 
            # temporary timeselector for min_max_graph
            dcc.Dropdown(
                id="select_year",
                options=[{"label": x, "value": x} for x in df["year"].unique()],
                value=2022,
